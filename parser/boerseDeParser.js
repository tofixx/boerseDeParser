...
const { childsByAttribute, getLink } = require('./helper');
...
async function createSummary(aktie, isin) {
	const uri = new URL(`/aktien/${aktie}/${isin}`, baseUrl);
	return request({ uri })
		.then(async (response) => {
...
module.exports = { createSampleChartUrls, createSummary };
